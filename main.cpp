#include <avr/io.h>
#include <avr/interrupt.h>
#include <avr/pgmspace.h>
#include <util/delay.h>

class interrupts_off {
public:
    interrupts_off() {
        was = SREG;
        cli();
    }
    ~interrupts_off() {
        SREG = was;
    }
    uint8_t was;
};

__attribute__((used, section(".bitswap_table"))) uint8_t const table[] = {
    ,
};

// BITSWAP_TABLE is defined in the Makefile as the address
// of section(".bitswap_table"). It must be on a 256-byte
// boundary.
//
// pgm_read_byte(table[x]) yields:
//     mov r30,r24
//     ldi r31,0
//     subi r30,lo8(-(_ZL5table))
//     sbci r31,hi8(-(_ZL5table))
//     ld r30,Z
//     ldi r31,0
//     lpm r24, Z
//     ret
//
// pgm_read_byte(BITSWAP_TABLE | x) yields:
//     mov r30,r24
//     ldi r31,0
//     ori r31,31
//     lpm r24, Z
static_assert(
    0 == (BITSWAP_TABLE & 0xff),
    "BITSWAP_TABLE is misaligned!");

uint8_t bitswap(uint8_t x)
{
    return pgm_read_byte(BITSWAP_TABLE | x);
}

void uart_write_2mbaud(uint8_t x) 
{
    interrupts_off const uninterrupted{};

    auto const swapped = bitswap(x);

    // I need one register for intermediate values assigned
    // in the r16-r31 range. I'd love to use __tmp_reg__ but
    // it's r0 and invalid as an operand for 'out'.
    uint8_t count = 0;
    asm volatile(
        // Start bit.
        "out %[usidr],%[count]\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"

        // Data bits.
        
        "out %[usidr],%[data]\n\t"
        "nop\n\t"
        "nop\n\t"
        "ldi %[count],7\n"

        "data_loop%=:\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "out %[usicr], %[shift]\n\t"
        "dec %[count]\n\t"
        "brne data_loop%=\n\t"

        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "ldi %[count],255\n\t"

        // Stop bit.
        "out %[usidr],%[count]\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        :   [count] "+d" (count)
        :   [usicr] "I" (_SFR_IO_ADDR(USICR)),
            [usidr] "I" (_SFR_IO_ADDR(USIDR)),
            [usisr] "I" (_SFR_IO_ADDR(USISR)),
            [clear] "d" (static_cast<uint8_t>(0xF0)),
            [shift] "d" (static_cast<uint8_t>(_BV(USIWM0) | _BV(USICLK))),
            [data] "d" (swapped)
    );
}

void uart_write_4mbaud(uint8_t x) 
{
    interrupts_off const uninterrupted{};

    auto const swapped = bitswap(x);

    // I need one register for intermediate values assigned
    // in the r16-r31 range. I'd love to use __tmp_reg__ but
    // it's r0 and invalid as an operand for 'out'.
    uint8_t count = 0;
    uint8_t const stop = 255;
    asm volatile(
        // Start bit.
        "out %[usidr],%[count]\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"

        // Data bits.
        
        "out %[usidr],%[data]\n\t"
        "nop\n\t"
        "nop\n\t"
        "ldi %[count],7\n"

        "data_loop%=:\n\t"
        "out %[usicr], %[shift]\n\t"
        "dec %[count]\n\t"
        "brne data_loop%=\n\t"

        // Stop bit.
        "out %[usidr],%[stop]\n\t"
        "nop\n\t"
        "nop\n\t"
        "nop\n\t"
        :   [count] "+d" (count)
        :   [usicr] "I" (_SFR_IO_ADDR(USICR)),
            [usidr] "I" (_SFR_IO_ADDR(USIDR)),
            [usisr] "I" (_SFR_IO_ADDR(USISR)),
            [clear] "d" (static_cast<uint8_t>(0xF0)),
            [shift] "d" (static_cast<uint8_t>(_BV(USIWM0) | _BV(USICLK))),
            [data] "d" (swapped),
            [stop] "d" (stop)
    );
}

void uart_write_8mbaud(uint8_t x) 
{
    interrupts_off const uninterrupted{};

    auto const swapped = bitswap(x);

    // I need one register for intermediate values assigned
    // in the r16-r31 range. I'd love to use __tmp_reg__ but
    // it's r0 and invalid as an operand for 'out'.
    uint8_t const start = 0;
    uint8_t const stop = 255;
    asm volatile(
        // Start bit.
        "out %[usidr],%[start]\n\t"
        "nop\n\t"

        // Data bits.
        
        "out %[usidr],%[data]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        "out %[usicr], %[shift]\n\t"
        "nop\n\t"

        // Stop bit.
        "out %[usidr],%[stop]\n\t"
        "nop\n\t"
        :
        :   [usicr] "I" (_SFR_IO_ADDR(USICR)),
            [usidr] "I" (_SFR_IO_ADDR(USIDR)),
            [usisr] "I" (_SFR_IO_ADDR(USISR)),
            [clear] "d" (static_cast<uint8_t>(0xF0)),
            [shift] "d" (static_cast<uint8_t>(_BV(USIWM0) | _BV(USICLK))),
            [data] "d" (swapped),
            [start] "d" (start),
            [stop] "d" (stop)
    );
}

void uart_write_16mbaud(uint8_t x) 
{
    interrupts_off const uninterrupted{};

    auto const swapped = bitswap(x);

    // I need one register for intermediate values assigned
    // in the r16-r31 range. I'd love to use __tmp_reg__ but
    // it's r0 and invalid as an operand for 'out'.
    uint8_t const start = 0;
    uint8_t const stop = 255;
    asm volatile(
        // Start bit.
        "out %[usidr],%[start]\n\t"

        // Data bits.
        "out %[usidr],%[data]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"
        "out %[usicr], %[shift]\n\t"

        // Stop bit.
        "out %[usidr],%[stop]\n\t"
        :
        :   [usicr] "I" (_SFR_IO_ADDR(USICR)),
            [usidr] "I" (_SFR_IO_ADDR(USIDR)),
            [usisr] "I" (_SFR_IO_ADDR(USISR)),
            [clear] "d" (static_cast<uint8_t>(0xF0)),
            [shift] "d" (static_cast<uint8_t>(_BV(USIWM0) | _BV(USICLK))),
            [data] "d" (swapped),
            [start] "d" (start),
            [stop] "d" (stop)
    );
}

void uart_write(char const* str)
{
    for (auto p = str; *p; ++p) {
        uart_write_2mbaud(*p);
    }
}

char to_hex_nibble(uint8_t lo)
{
    return lo < 0xa ? ('0' + lo) : ('a' + (lo - 0xa));
}

uint8_t hi(uint8_t x)
{
    return x >> 4;
}

uint8_t lo(uint8_t x)
{
    return x & 0xf;
}

int main(void)
{
    // Adjust OSCCAL to get as close to an accurate 16 MHz as possible.
    // Set CKOUT fuse to put the CPU clock on a pin for measurement.
    OSCCAL -= 2;

    USIDR = 0xFF;
    USICR = _BV(USIWM0);
    DDRB |= _BV(PB1);

    uint8_t data[3] { 0, 0, 0 };
    while (true) {
        char buf[] {
            to_hex_nibble(hi(data[2])),
            to_hex_nibble(lo(data[2])),
            to_hex_nibble(hi(data[1])),
            to_hex_nibble(lo(data[1])),
            to_hex_nibble(hi(data[0])),
            to_hex_nibble(lo(data[0])),
            '\r',
            '\n',
            '\0'
        };
        uart_write(buf);

        data[2] = data[1];
        data[1] = data[0];
        ++data[0];
        _delay_ms(1000);
    }
}
